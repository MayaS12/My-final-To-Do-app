...
fetchTasks = () => {
    firebase
      .database()
      .ref('/users/'+firebase.auth().currentUser.uid+'/tasks/'+'Home To - Do List!')
      .on('value', (snapshot) => { 
        let tasks = [];  
        if (snapshot.val()) {
...
console.log(taskKey); 
firebase .database() .ref( '/users/' + firebase.auth().currentUser.uid + '/tasks/' + '/Home To - Do List!/' + taskKey ) .remove(); };
